...
const p = 'src/components/AdminDashboardClient.tsx';
let code = fs.readFileSync(p, 'utf8');

// 1. Add imports
if (!code.includes('import { getMessages, deleteMessage }')) {
  code = code.replace(
...
// 2. Add state
if (!code.includes('const [messages, setMessages]')) {
  code = code.replace(
...
if (!code.includes('"INBOX"')) {
  code = code.replace(
    'const tabs = ["PROFILE", "SKILLS", "PROJECTS", "CERTIFICATIONS", "EDUCATION", "RESEARCH"];',
    'const tabs = ["PROFILE", "SKILLS", "PROJECTS", "CERTIFICATIONS", "EDUCATION", "RESEARCH", "INBOX"];'
  );
}

// 4. Add INBOX tab rendering
const inboxHtml = `
          {/* INBOX TAB */}
          {activeTab === "INBOX" && (
...
if (!code.includes('activeTab === "INBOX"')) {
  code = code.replace(
    '        </div>\n      </div>\n    </div>\n  );\n}',
    inboxHtml
  );
...
console.log('AdminDashboardClient.tsx updated successfully');
